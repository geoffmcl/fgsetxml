...
#ifndef DEF_TEST1_FILE
#define DEF_TEST1_FILE "../samples/PMMLModel.xml"
#endif
#ifndef DEF_TEST2_FILE
#define DEF_TEST2_FILE "../samples/utf8test.xml"
#endif

const char *file1 = DEF_TEST1_FILE;
const char *file2 = DEF_TEST2_FILE;

void myfree(char *t); // {free(t);}
ToXMLStringTool tx,tx2;

int check_files_exists()
{
    struct stat buf;
    int iret = 0;
    if (stat(file1, &buf)) {
        printf("Error: Am unable to 'stat' '%s'\n", file1);
        iret++;
    }
    if (stat(file2, &buf)) {
        printf("Error: Am unable to 'stat' '%s'\n", file2);
...
int main(int argc, char **argv)
{
    ...
    XMLNode xMainNode=XMLNode::openFileHelper(file1,"PMML");
    ...
    xMainNode.getChildNode("Extension").writeToFile("temptest.xml","ISO-8859-1");
    ...
    xMainNode=XMLNode::openFileHelper(file1,"PMML").getChildNode("Header");
    ...
    xMainNode=XMLNode::openFileHelper(file2);
    xMainNode.writeToFile("tempTestUTF8.xml");
    printf("  ... resulting multi-lingual file is 'tempTestUTF8.xml'.\n\n");
